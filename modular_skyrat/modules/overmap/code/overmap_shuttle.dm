...
/datum/overmap_object/shuttle/ui_interact(mob/user, datum/tgui/ui, turf/usage_turf = 0)
	if(usage_turf)
			...
			if(VECTOR_LENGTH(velocity_x, velocity_y) > SHUTTLE_MAXIMUM_DOCKING_SPEED)
				...
				if(gateways.len)
					...
					for(var/key in gateways)
						dat += "<BR> - [key] - <a href='?src=[REF(src)];task=dock;dock_control=gateway_dock;dock_id=[gateways[key].shuttle_id]'>Enter Wormhole</a>"
...
/datum/overmap_object/shuttle/Topic(href, href_list)
	...
	if(!control_turf)
		return
	var/mob/user = usr
	if(!isliving(user) || get_dist(user, control_turf) > 2)
