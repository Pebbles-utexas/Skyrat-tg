...
/obj/machinery/computer/trade_console/Topic(href, href_list)
	...
	if(!istype(living_user) || !living_user.can_perform_action(src, FORBID_TELEKINESIS_REACH))
		return
				...
				if("interact_with_bounty")
					...
					switch(href_list["bounty_type"])
						if("print")
							...
							P.add_raw_text("<CENTER><B>[connected_trader.origin] - BOUNTY: [bounty.bounty_name]</B></CENTER><HR>")
